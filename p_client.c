...
void TossClientWeapon (edict_t *self)
{
	...
	if (quad)//While quad is on the ground from a player dying this continues to run Reddragoon
	{
		...
		drop->nextthink = 450;//This controls the time outside the player Reddragoon
...
void InitClientPersistant (gclient_t *client)
{
	...
	quadtime = 1.0;
	client->quadwinner = false;//Reddragoon
...
void ClientThink (edict_t *ent, usercmd_t *ucmd)
{
	...
	for (i=0 ; i<maxclients->value ; i++)//check every client on the server
		{
	...
	if(quadtime <= 0.5)//Reddragoon
	{
		client->quadwinner = true;
	}
	else
		client->quadwinner = false;
